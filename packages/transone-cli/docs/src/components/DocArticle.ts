...
export class DocArticle extends Component<{ blocks: DocBlock[] }, object> {
  ...
  private renderLink(text: string, href: string): VNode {
    if (href.startsWith('./')) {
      // 站点相对链接：指向主站根（如 /transone/）
      return {
        tag: 'a',
        props: { href: withSiteRootPath(href.slice(1)), className: 'doc-external' },
        children: [text],
      };
    }
    if (href.startsWith('/')) {
      // 站内链接：普通 <a> + 本站 base 前缀（同 TSone 文档站）
      return {
        tag: 'a',
  ...
  private renderCard(
    ...
    if (href.startsWith('./')) {
      return {
        tag: 'a',
        props: { href: withSiteRootPath(href.slice(1)), className: 'doc-link-card' },
        children,
      };
    }
    return {
  ...
  private renderButton(
    ...
    if (href.startsWith('./')) {
      return {
        tag: 'a',
        props: { href: withSiteRootPath(href.slice(1)), className },
        children: [label],
      };
    }
    return {
