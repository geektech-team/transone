 ...
 * 且满足段边界（路由前是空或 '/'），则剥掉路由后的剩余部分即 base。
...
/**
 * 主文档站路径（CLI 子站的父级站点）。
 * CLI 子站部署在 /transone/cli/，主站在 /transone/：去掉 base 最后一段即可。
 */
export function parentDocBasePath(base: string): string {
  const normalized = normalizeDocBasePath(base);
  if (!normalized) {
    return '';
  }
...
export function withSiteRootPath(path: string): string {
  const root = parentDocBasePath(getDocBasePath());
  if (!root) {
    return path;
  }
