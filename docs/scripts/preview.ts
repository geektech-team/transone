...
const port = Number(process.env.PORT ?? 52313);

if (!existsSync(outDir)) {
  console.error(`未找到构建产物 ${outDir}。请先运行：bun run --cwd docs build:web`);
  process.exit(1);
}

// 从产物 index.html 读取部署 base（--base 构建产物为 /transone 等），
...
const server = Bun.serve({
  ...
  fetch(request) {
    ...
    let rel = pathname === '/' ? '/index.html' : pathname.slice(base.length) || '/index.html';
...
console.log(
  `TransOne docs preview server: http://localhost:${server.port}${base ? ` (base ${base})` : ''}`
);
